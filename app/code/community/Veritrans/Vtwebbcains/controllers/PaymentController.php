...
class Veritrans_Vtwebbcains_PaymentController
...
/*    $order->setState(Mage::getStoreConfig('payment/vtweb/'),true,
    ...
    $enable_installment = Mage::getStoreConfig('payment/vtwebbcains/enable_installment');
    // $is_enabled_mandiri = Mage::getStoreConfig('payment/vtwebmandiri/enable_installment_mandiri');

    Veritrans_Config::$is3ds = true;

    Veritrans_Config::$isProduction =
        Mage::getStoreConfig('payment/vtwebbcains/environment') == 'production'
        ? true : false;

    Veritrans_Config::$serverKey =
        Mage::getStoreConfig('payment/vtwebbcains/server_key');
    
    Veritrans_Config::$is3ds = true;

    Veritrans_Config::$isSanitized =
        Mage::getStoreConfig('payment/vtwebbca/enable_sanitized') == '1'
        ? true : false; 

    error_log(Mage::getStoreConfig('payment/vtwebbcains/environment'));
    error_log(Mage::getStoreConfig('payment/vtwebbcains/server_key'));
    ...
    Mage::log('$threshold = '.print_r($threshold,true),null,'vtwebbcains.log',true);
    ...
    if($bin_list)
    {
      ...
      Mage::log('bin list = '.print_r($bin_list_array,true),null,'vtwebbcains.log',true);
    }
